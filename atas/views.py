...
def detalhe_ata(request, slug):
    ata = get_object_or_404(Ata, slug=slug)
...
def criar_ata(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ata.slug = slugify(ata.titulo)
            ata.save()
            messages.success(request, 'Ata criada com sucesso!')
            return redirect('atas:detalhe_ata', slug=ata.slug)
    else:
        form = AtaForm()
